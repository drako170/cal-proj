...
public class ViewLogController implements Initializable {
	...
	@FXML
	TableView<Table> logTable = new TableView<Table>();
	...
	private ObservableList<Table> getReminders(){
		...
		DLLNode<Reminder> node = ReminderApplication.log.getHeader();
		
		
		for(int i = 0; i < ReminderApplication.log.getSize(); i++) {
	 				   ...
	 				   while(root != null){
		 				   if(root.getInfo().getFull().equals(s)){
		 					   ...
		 					   if(next == null && pre == null){
		 						   ReminderApplication.log.header = null;
		 						  ReminderApplication.log.trailer = null;
		 					   }
		 					   else if(next == null){
		 						   ReminderApplication.log.removeLast();
		 					   }
		 					   else if(pre == null){
		 						   root.setLink(null);
		 						   next.setBack(pre);
		 						   ReminderApplication.log.header = next;
		 						   ReminderApplication.log.size--;
		 					   }
		 					   else{
		 						   pre.setLink(next);
		 						   next.setBack(pre);
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		logTable.setItems(getReminders());
